...
export const BlockEditorV2 = ({
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      try {
        ...
        const { data, name, editor: existingEditor, keywords, programmeId } = await res.json();
  ...
  const saveContent = async () => {
    ...
    try {
        const url = dataChunkId ? `/api/data-chunk/${dataChunkId}` : "/api/data-chunk";
        ...
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            metaData: { // Move metadata inside `metaData`
                editor: editorName,
