...
import { IncomingInnovationData } from '../interfaces-incoming';
...
export type WebflowInnovationFormData = z.infer<typeof innovationSchema>;

interface WebflowInnovationFormProps {
  initialData?: Partial<IncomingInnovationData>;
  onSubmit: (data: IncomingInnovationData) => Promise<void>;
  onCancel: () => void;
  onDelete?: () => Promise<void>;
  isEditing?: boolean;
}

export interface WebflowInnovationFormRef {
  triggerSubmit: () => void;
  setStatus: (status: 'draft' | 'published') => void;
}

function toIncoming(data: WebflowInnovationFormData): IncomingInnovationData {
  return {
    title: data.title,
...
export const WebflowInnovationForm = forwardRef<
  WebflowInnovationFormRef,
  WebflowInnovationFormProps
>(({ initialData, onSubmit, onCancel, onDelete, isEditing = false }, ref) => {
  ...
  const form = useForm<WebflowInnovationFormData>({
  ...
  const handleGenerateSlug = () => {
    ...
    if (currentTitle) {
      ...
      const uniqueSlug = `${baseSlug}-innovation`;

      form.setValue('slug', uniqueSlug);
    }
  };

  const handleSubmit = async (data: WebflowInnovationFormData) => {
    ...
    try {
      ...
      console.error('Innovation form submit error:', error);
      ...
      throw new Error(
        'An unexpected error occurred while saving the innovation.'
      );
    } finally {
      setIsLoading(false);
      setBusyAction(null);
  ...
  return (
    <div className="h-full flex flex-col prevent-layout-shift">
      ...
      <div className="sticky top-0 z-10 bg-gray-900 border-b border-gray-700 p-4 -mx-6 -mt-6 mb-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-white">
            {isEditing ? 'Edit' : 'Create'} Innovation
          </h3>
      ...
      <Dialog open={showCancelConfirm} onOpenChange={setShowCancelConfirm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cancel Innovation</DialogTitle>
            <DialogDescription>
...
WebflowInnovationForm.displayName = 'WebflowInnovationForm';
