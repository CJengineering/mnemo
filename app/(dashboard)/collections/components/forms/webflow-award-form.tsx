...
import { IncomingAwardData } from '../interfaces-incoming';
...
export type WebflowAwardFormData = z.infer<typeof awardSchema>;

interface WebflowAwardFormProps {
  initialData?: Partial<IncomingAwardData>;
  onSubmit: (data: IncomingAwardData) => Promise<void>;
  onCancel: () => void;
  onDelete?: () => Promise<void>;
  isEditing?: boolean;
}

export interface WebflowAwardFormRef {
  triggerSubmit: () => void;
  setStatus: (status: 'draft' | 'published') => void;
}

function toIncoming(data: WebflowAwardFormData): IncomingAwardData {
  return {
    title: data.title,
...
export const WebflowAwardForm = forwardRef<
  WebflowAwardFormRef,
  WebflowAwardFormProps
>(({ initialData, onSubmit, onCancel, onDelete, isEditing = false }, ref) => {
  ...
  const form = useForm<WebflowAwardFormData>({
  ...
  const handleGenerateSlug = () => {
    ...
    if (currentTitle) {
      ...
      const uniqueSlug = `${baseSlug}-award`;

      form.setValue('slug', uniqueSlug);
    }
  };

  const handleSubmit = async (data: WebflowAwardFormData) => {
    ...
    try {
      ...
      console.error('Award form submit error:', error);
      ...
      throw new Error('An unexpected error occurred while saving the award.');
    } finally {
      setIsLoading(false);
      setBusyAction(null);
    }
  ...
  return (
    <div className="h-full flex flex-col prevent-layout-shift">
      ...
      <Dialog open={showCancelConfirm} onOpenChange={setShowCancelConfirm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cancel Award</DialogTitle>
            <DialogDescription>
...
WebflowAwardForm.displayName = 'WebflowAwardForm';
