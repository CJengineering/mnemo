...
/**
 * Post Images CDN Migration Script
 *
 * This script fetches post collection items from the external Mnemo database,
 * downloads images (including OpenGraph), uploads them to Google Cloud Storage,
 * and updates the URLs to point to cdn.communityjameel.org
 *
 * Usage: node migrate-post-images-to-cdn.mjs [--limit=5] [--dry-run]
 *
 * Examples:
 * - node migrate-post-images-to-cdn.mjs --limit=5     # Process first 5 posts for testing
 * - node migrate-post-images-to-cdn.mjs --dry-run     # Preview what would be migrated
 * - node migrate-post-images-to-cdn.mjs               # Process all posts
 */
...
const CONFIG = {
  ...
  // External Mnemo API Configuration
  MNEMO_API_BASE: 'https://mnemo-app-e4f6j5kdsq-ew.a.run.app',

  // Google Cloud Storage credentials from env
  PRIVATE_GCL: process.env.PRIVATE_GCL || ''
};
...
console.log('🚀 Post Images CDN Migration Script');
console.log('=====================================');
...
if (!isDryRun) {
  try {
    ...
    const privateKey = Buffer.from(CONFIG.PRIVATE_GCL, 'base64').toString(
      'utf8'
    );
  ...
  console.log('⏩ Skipping GCS initialization (dry run mode)');
}
...
async function processImage(imageUrl, imageType, postSlug, index = null) {
  ...
  const gcsPath = `website/collection/post/${postSlug}/${fileName}`;
...
async function main() {
  try {
    ...
    const reportFileName = `post-images-cdn-migration-report-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
